...
FROM Cuenta C INNER JOIN (
    ...
    FROM(
            ...
            SELECT DISTINCT TOP 10
            Me.IdCuenta, Me.ImporteMovim - Ms.ImporteMovim AS 'saldo'
        FROM Movimiento Me INNER JOIN Movimiento Ms ON Me.IdCuenta = Ms.IdCuenta
        WHERE
                Me.TipoMovim = 'E'
            AND Ms.TipoMovim <> 'E'
...
SELECT DISTINCT
    (SELECT SUM (Md.ImporteMovim)
    FROM Movimiento Md
    WHERE Md.TipoMovim = 'E') AS 'Depositos',
    (SELECT SUM (Mr.ImporteMovim)
    FROM Movimiento Mr
    WHERE Mr.TipoMovim <> 'S') AS 'Retiros',
    (SELECT AVG (Mp.ImporteMovim)
    FROM Movimiento Mp) AS 'Promedio'
FROM MOVIMIENTO M
